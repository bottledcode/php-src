	...
	if (ce->constructor) {
		if (parent->constructor && UNEXPECTED(parent->constructor->common.fn_flags & ZEND_ACC_FINAL)) {
			zend_error_noreturn(E_ERROR, "Cannot override final %s::%s() with %s::%s()",
				ZSTR_VAL((zend_string *)parent->name), ZSTR_VAL(parent->constructor->common.function_name),
				ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(ce->constructor->common.function_name));
		}
		return;
	}
...
static zend_string *resolve_class_name(const zend_class_entry *scope, zend_string *name) {
	...
	if (zend_string_equals_ci(name, ZSTR_KNOWN(ZEND_STR_PARENT)) && scope->parent) {
		if (scope->ce_flags & ZEND_ACC_RESOLVED_PARENT) {
			return (zend_string *)scope->parent->name;
		} else {
			return (zend_string *)scope->parent_name;
		}
	} else if (zend_string_equals_ci(name, ZSTR_KNOWN(ZEND_STR_SELF))) {
		return (zend_string *)scope->name;
	} else {
		return name;
	}
}
...
static zend_class_entry *lookup_class_ex(
	...
	if (UNEXPECTED(!EG(active) && !in_preload)) {
		...
		if (register_unresolved && !ce) {
			zend_error_noreturn(
				...
				ZSTR_VAL(name), ZSTR_VAL((zend_string *)scope->name));
	    }
	...
	if (!CG(in_compilation) || in_preload) {
		...
		if (zend_string_equals_ci((zend_string *)scope->name, name)) {
			return scope;
		}
	}
		...
		const zend_function *fptr, zend_class_entry *scope) /* {{{ */
{
	...
	if (fptr->common.scope) {
		if (fptr->common.scope->ce_flags & ZEND_ACC_ANON_CLASS) {
			...
			smart_str_appends(&str, ZSTR_VAL((zend_string *)fptr->common.scope->name));
		} else {
			smart_str_appendl(&str, ZSTR_VAL((zend_string *)fptr->common.scope->name), ZSTR_LEN((zend_string *)fptr->common.scope->name));
		}
		smart_str_appends(&str, "::");
	}
		...
		if (!return_type_will_change_attribute) {
			...
			if (EG(exception)) {
				zend_exception_uncaught_error(
					"During inheritance of %s", ZSTR_VAL((zend_string *)parent_scope->name));
			}
		}
	} else {
		zend_error_at(E_COMPILE_ERROR, func_filename(child), func_lineno(child),
...
static void inherit_property_hook(
	...
	if (parent_flags & ZEND_ACC_FINAL) {
		zend_error_noreturn(E_COMPILE_ERROR,
			...
			ZSTR_VAL((zend_string *)parent->common.scope->name),
	...
	if (UNEXPECTED(child)) {
		...
		if (parent_info->flags & ZEND_ACC_FINAL) {
			zend_error_noreturn(E_COMPILE_ERROR, "Cannot override final property %s::$%s",
				ZSTR_VAL((zend_string *)parent_info->ce->name), ZSTR_VAL(key));
		}
		if (!(parent_info->flags & ZEND_ACC_PRIVATE)) {
			...
			if (UNEXPECTED((parent_info->flags & ZEND_ACC_STATIC) != (child_info->flags & ZEND_ACC_STATIC))) {
				zend_error_noreturn(E_COMPILE_ERROR, "Cannot redeclare %s%s::$%s as %s%s::$%s",
					(parent_info->flags & ZEND_ACC_STATIC) ? "static " : "non static ", ZSTR_VAL((zend_string *)parent_info->ce->name), ZSTR_VAL(key),
					(child_info->flags & ZEND_ACC_STATIC) ? "static " : "non static ", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(key));
			}
			if (UNEXPECTED((child_info->flags & ZEND_ACC_READONLY) != (parent_info->flags & ZEND_ACC_READONLY))) {
				if (!(parent_info->flags & ZEND_ACC_ABSTRACT)) {
					zend_error_noreturn(E_COMPILE_ERROR,
						...
						ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(key));
				}
			}
			 ...
			 && !(parent_info->hooks && (parent_info->flags & ZEND_ACC_VIRTUAL) && !parent_info->hooks[ZEND_PROPERTY_HOOK_SET])) {
				...
				if (child_set_visibility > parent_set_visibility) {
					zend_error_noreturn(
						...
						ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(key),
						zend_asymmetric_visibility_string(parent_info->flags), ZSTR_VAL((zend_string *)parent_info->ce->name),
						!(parent_info->flags & ZEND_ACC_PPP_SET_MASK) ? "" : " or weaker");
				}
			}

			if (UNEXPECTED((child_info->flags & ZEND_ACC_PPP_MASK) > (parent_info->flags & ZEND_ACC_PPP_MASK))) {
				zend_error_noreturn(E_COMPILE_ERROR, "Access level to %s::$%s must be %s (as in class %s)%s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(key), zend_visibility_string(parent_info->flags), ZSTR_VAL((zend_string *)parent_info->ce->name), (parent_info->flags&ZEND_ACC_PUBLIC) ? "" : " or weaker");
			}
			...
			if (ZEND_TYPE_IS_SET(parent_info->type)) {
				...
				zend_error_noreturn(E_COMPILE_ERROR,
						...
						ZSTR_VAL(key),
						ZSTR_VAL((zend_string *)parent_info->ce->name));
			}
		}
	} else {
...
static inline void do_implement_interface(zend_class_entry *ce, zend_class_entry *iface) /* {{{ */
{
	if (!(ce->ce_flags & ZEND_ACC_INTERFACE) && iface->interface_gets_implemented && iface->interface_gets_implemented(iface, ce) == FAILURE) {
		zend_error_noreturn(E_CORE_ERROR, "%s %s could not implement interface %s", zend_get_object_type_uc(ce), ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)iface->name));
	}
...
ZEND_API void zend_verify_hooked_property(zend_class_entry *ce, zend_property_info *prop_info, zend_string *prop_name)
{
	...
	if ((prop_info->flags & ZEND_ACC_VIRTUAL) && prop_info->offset != ZEND_VIRTUAL_PROPERTY_OFFSET) {
		if (Z_TYPE(ce->default_properties_table[OBJ_PROP_TO_NUM(prop_info->offset)]) == IS_UNDEF) {
			...
			zend_error_noreturn(E_COMPILE_ERROR,
				"Cannot specify default value for virtual hooked property %s::$%s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(prop_name));
		}
	}
			...
			if ((zend_property_hook_kind)i == ZEND_PROPERTY_HOOK_GET
			 ...
			 && prop_info->hooks[ZEND_PROPERTY_HOOK_SET]) {
				zend_error_noreturn(E_COMPILE_ERROR, "Get hook of backed property %s::%s with set hook may not return by reference",
					ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(prop_name));
			}
	...
	if (abstract_error) {
		zend_error_noreturn(E_COMPILE_ERROR,
			"Abstract property %s::$%s must specify at least one abstract hook", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(prop_name));
	}
	if ((prop_info->flags & ZEND_ACC_VIRTUAL)
	 ...
	 && (!prop_info->hooks[ZEND_PROPERTY_HOOK_GET] || !prop_info->hooks[ZEND_PROPERTY_HOOK_SET])) {
		...
		zend_error_noreturn(E_COMPILE_ERROR,
			...
			prefix, ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(prop_name));
	}
}
...
ZEND_API ZEND_COLD ZEND_NORETURN void zend_hooked_property_variance_error(const zend_property_info *prop_info)
{
	...
	zend_hooked_property_variance_error_ex(value_param_name, (zend_string *)prop_info->ce->name, prop_info->name);
}
...
ZEND_API void zend_do_inheritance_ex(zend_class_entry *ce, zend_class_entry *parent_ce, bool checked) /* {{{ */
{
	...
	if (UNEXPECTED(ce->ce_flags & ZEND_ACC_INTERFACE)) {
		...
		if (UNEXPECTED(!(parent_ce->ce_flags & ZEND_ACC_INTERFACE))) {
			zend_error_noreturn(E_COMPILE_ERROR, "Interface %s cannot extend class %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)parent_ce->name));
		}
		...
		if (parent_ce->ce_flags & ZEND_ACC_ENUM) {
			zend_error_noreturn(E_COMPILE_ERROR, "Class %s cannot extend enum %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)parent_ce->name));
		}
		/* Class must not extend a final class */
		if (parent_ce->ce_flags & ZEND_ACC_FINAL) {
			zend_error_noreturn(E_COMPILE_ERROR, "Class %s cannot extend final class %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)parent_ce->name));
		}
		...
		if ((parent_ce->ce_flags & ZEND_ACC_INTERFACE) || (parent_ce->ce_flags & ZEND_ACC_TRAIT)) {
			zend_error_noreturn(E_COMPILE_ERROR, "Class %s cannot extend %s %s",
				ZSTR_VAL((zend_string *)ce->name), parent_ce->ce_flags & ZEND_ACC_INTERFACE ? "interface" : "trait", ZSTR_VAL((zend_string *)parent_ce->name)
			);
		}
	}
	...
	if (UNEXPECTED((ce->ce_flags & ZEND_ACC_READONLY_CLASS) != (parent_ce->ce_flags & ZEND_ACC_READONLY_CLASS))) {
		zend_error_noreturn(E_COMPILE_ERROR, "%s class %s cannot extend %s class %s",
			ce->ce_flags & ZEND_ACC_READONLY_CLASS ? "Readonly" : "Non-readonly", ZSTR_VAL((zend_string *)ce->name),
			parent_ce->ce_flags & ZEND_ACC_READONLY_CLASS ? "readonly" : "non-readonly", ZSTR_VAL((zend_string *)parent_ce->name)
		);
	}
...
static bool do_inherit_constant_check(
	...
	if (parent_constant->ce != child_constant->ce && (ZEND_CLASS_CONST_FLAGS(parent_constant) & ZEND_ACC_FINAL)) {
		zend_error_noreturn(E_COMPILE_ERROR, "%s::%s cannot override final constant %s::%s",
			ZSTR_VAL((zend_string *)child_constant->ce->name), ZSTR_VAL(name),
			ZSTR_VAL((zend_string *)parent_constant->ce->name), ZSTR_VAL(name)
		);
	}
	...
	for (i = 0; i < ce->num_interfaces; i++) {
		if (ce->interfaces[i] == NULL) {
			...
			if (EXPECTED(i < parent_iface_num)) {
				...
				zend_error_noreturn(E_COMPILE_ERROR, "Class %s cannot implement previously implemented interface %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)iface->name));
			}
		}
	}
	if (ignore) {
...
static void zend_do_implement_interfaces(zend_class_entry *ce, zend_class_entry **interfaces) /* {{{ */
{
	...
	for (i = 0; i < ce->num_interfaces; i++) {
		...
		if (UNEXPECTED(!(iface->ce_flags & ZEND_ACC_INTERFACE))) {
			efree(interfaces);
			zend_error_noreturn(E_ERROR, "%s cannot implement %s - it is not an interface", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)iface->name));
			return;
		}
			...
			if (interfaces[j] == iface) {
				if (j >= num_parent_interfaces) {
					...
					zend_error_noreturn(E_COMPILE_ERROR, "%s %s cannot implement previously implemented interface %s",
						zend_get_object_type_uc(ce),
						ZSTR_VAL((zend_string *)ce->name),
						ZSTR_VAL((zend_string *)iface->name));
					return;
				}
				/* skip duplications */
				ZEND_HASH_MAP_FOREACH_STR_KEY_PTR(&iface->constants_table, key, c) {
	...
	if ((existing_fn = zend_hash_find_ptr(&ce->function_table, key)) != NULL) {
		...
		if (existing_fn->common.scope == ce) {
			...
			zend_error_noreturn(E_COMPILE_ERROR, "Trait method %s::%s has not been applied as %s::%s, because of collision with %s::%s",
				ZSTR_VAL((zend_string *)fn->common.scope->name), ZSTR_VAL(fn->common.function_name),
				ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(name),
				ZSTR_VAL((zend_string *)existing_fn->common.scope->name), ZSTR_VAL(existing_fn->common.function_name));
		} else {
...
static uint32_t zend_check_trait_usage(zend_class_entry *ce, zend_class_entry *trait, zend_class_entry **traits) /* {{{ */
{
	...
	if (UNEXPECTED((trait->ce_flags & ZEND_ACC_TRAIT) != ZEND_ACC_TRAIT)) {
		zend_error_noreturn(E_COMPILE_ERROR, "Class %s is not a trait, Only traits may be used in 'as' and 'insteadof' statements", ZSTR_VAL((zend_string *)trait->name));
		return 0;
	}
	...
	zend_error_noreturn(E_COMPILE_ERROR, "Required Trait %s wasn't added to %s", ZSTR_VAL((zend_string *)trait->name), ZSTR_VAL((zend_string *)ce->name));
	return 0;
}
/* }}} */
		...
		while ((cur_precedence = precedences[i])) {
			...
			if (!zend_hash_exists(&trait->function_table, lcname)) {
				zend_error_noreturn(E_COMPILE_ERROR,
						   ...
						   ZSTR_VAL((zend_string *)trait->name),
			...
			for (j = 0; j < cur_precedence->num_excludes; j++) {
				...
				if (zend_hash_add_empty_element(exclude_tables[trait_num], lcname) == NULL) {
					zend_error_noreturn(E_COMPILE_ERROR, "Failed to evaluate a trait precedence (%s). Method of trait %s was defined to be excluded multiple times", ZSTR_VAL(precedences[i]->trait_method.method_name), ZSTR_VAL((zend_string *)exclude_ce->name));
				}
	...
	if (ce->trait_aliases) {
		...
		while (ce->trait_aliases[i]) {
			...
			if (cur_method_ref->class_name) {
				...
				if (!zend_hash_exists(&trait->function_table, lcname)) {
					zend_error_noreturn(E_COMPILE_ERROR, "An alias was defined for %s::%s but this method does not exist", ZSTR_VAL((zend_string *)trait->name), ZSTR_VAL(cur_method_ref->method_name));
				}
			} else {
					...
					if (traits[j]) {
						if (zend_hash_exists(&traits[j]->function_table, lcname)) {
							...
							zend_error_noreturn(E_COMPILE_ERROR,
								...
								ZSTR_VAL(cur_method_ref->method_name),
								ZSTR_VAL((zend_string *)trait->name), ZSTR_VAL((zend_string *)traits[j]->name),
								ZSTR_VAL((zend_string *)trait->name), ZSTR_VAL(cur_method_ref->method_name),
								ZSTR_VAL((zend_string *)traits[j]->name), ZSTR_VAL(cur_method_ref->method_name));
						}
					}
				}
...
static void emit_incompatible_trait_constant_error(
	...
	zend_error_noreturn(E_COMPILE_ERROR,
		...
		ZSTR_VAL((zend_string *)find_first_constant_definition(ce, traits, current_trait, name, existing_constant->ce)->name),
		ZSTR_VAL((zend_string *)trait_constant->ce->name),
		ZSTR_VAL(name),
		ZSTR_VAL((zend_string *)ce->name)
	);
}
				...
				if ((colliding_prop->flags & ZEND_ACC_PRIVATE) && colliding_prop->ce != ce) {
					...
					if (colliding_prop->hooks || property_info->hooks) {
						zend_error_noreturn(E_COMPILE_ERROR,
							...
							ZSTR_VAL((zend_string *)find_first_property_definition(ce, traits, i, prop_name, colliding_prop->ce)->name),
							ZSTR_VAL((zend_string *)property_info->ce->name),
							ZSTR_VAL(prop_name),
							ZSTR_VAL((zend_string *)ce->name));
					}
	...
	if (ai.cnt) {
		if (!is_explicit_abstract && can_be_abstract) {
			zend_error_noreturn(E_ERROR,
				...
				zend_get_object_type_uc(ce),
				ZSTR_VAL((zend_string *)ce->name), ai.cnt,
				ai.cnt > 1 ? "s" : "",
				ai.cnt > 1 ? "s" : "",
...
static void check_unrecoverable_load_failure(const zend_class_entry *ce) {
	...
	if (CG(unlinked_uses)
		...
		zend_exception_uncaught_error(
			"During inheritance of %s with variance dependencies", ZSTR_VAL((zend_string *)ce->name));
	}
}
...
ZEND_API zend_class_entry *zend_do_link_class(zend_class_entry *ce, zend_string *lc_parent_name, zend_string *key) /* {{{ */
{
	...
	if (ce->parent_name) {
		parent = zend_fetch_class_by_name(
			(zend_string *)ce->parent_name, lc_parent_name,
	...
	if (ce->num_traits || ce->num_interfaces) {
		...
		for (i = 0; i < ce->num_traits; i++) {
			...
			if (UNEXPECTED(!(trait->ce_flags & ZEND_ACC_TRAIT))) {
				zend_throw_error(NULL, "%s cannot use %s - it is not a trait", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)trait->name));
				free_alloca(traits_and_interfaces, use_heap);
				return NULL;
			}
			for (j = 0; j < i; j++) {
	...
	if (ZSTR_HAS_CE_CACHE((zend_string *)ce->name)) {
		ZSTR_SET_CE_CACHE((zend_string *)ce->name, ce);
	}
