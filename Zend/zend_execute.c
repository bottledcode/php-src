...
static zend_never_inline ZEND_COLD void zend_throw_auto_init_in_prop_error(zend_property_info *prop) {
	...
	zend_type_error(
		...
		ZSTR_VAL((zend_string *)prop->ce->name), zend_get_unmangled_property_name(prop->name),
		ZSTR_VAL(type_str)
	);
...
static zend_never_inline ZEND_COLD void zend_throw_access_uninit_prop_by_ref_error(
	...
	zend_throw_error(NULL,
		...
		ZSTR_VAL((zend_string *)prop->ce->name),
		...
		zend_string **need_msg, const char **given_kind)
{
	*fname = ZSTR_VAL(zf->common.function_name);
	if (zf->common.scope) {
		*fsep =  "::";
		*fclass = ZSTR_VAL((zend_string *)zf->common.scope->name);
	} else {
		*fsep =  "";
		*fclass = "";
	}
...
ZEND_COLD zend_never_inline void zend_verify_class_constant_type_error(const zend_class_constant *c, const zend_string *name, const zval *constant)
{
	...
	zend_type_error("Cannot assign %s to class constant %s::%s of type %s",
		zend_zval_type_name(constant), ZSTR_VAL((zend_string *)c->ce->name), ZSTR_VAL(name), ZSTR_VAL(type_str));
...
ZEND_COLD zend_never_inline void zend_verify_property_type_error(const zend_property_info *info, const zval *property)
{
	...
	zend_type_error("Cannot assign %s to property %s::$%s of type %s",
		...
		ZSTR_VAL((zend_string *)info->ce->name),
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_readonly_property_modification_error(
	...
	zend_readonly_property_modification_error_ex(
		ZSTR_VAL((zend_string *)info->ce->name), zend_get_unmangled_property_name(info->name));
}
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_asymmetric_visibility_property_modification_error(
	...
	zend_throw_error(NULL, "Cannot %s %s property %s::$%s from %s%s",
		...
		scope ? "scope " : "global scope", scope ? ZSTR_VAL((zend_string *)scope->name) : "");
}
		...
		if (UNEXPECTED(ZEND_TYPE_HAS_LIST(*type))) {
			...
			if (ce && ce->required_scope) {
				...
				if (ce->required_scope_absolute && scope != ce->required_scope) {
					if (scope == NULL) {
						zend_error(E_ERROR, "Private inner class %s cannot be used as a type declaration in the global scope", ZSTR_VAL((zend_string *)ce->name));
					}

					zend_error(E_ERROR, "Private inner class %s cannot be used as a type declaration in the scope of %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)scope->name));
				} else if (scope == NULL) {
					zend_error(E_ERROR, "Protected inner class %s cannot be used as a type declaration in the global scope", ZSTR_VAL((zend_string *)ce->name));
				} else if (!instanceof_function(scope, ce->required_scope)) {
					zend_error(E_ERROR, "Protected inner class %s cannot be used as a type declaration in the scope of %s", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL((zend_string *)scope->name));
				}
			}
...
ZEND_API ZEND_COLD void zend_internal_call_arginfo_violation(zend_function *fbc)
{
	zend_error_noreturn(E_ERROR, "Arginfo / zpp mismatch during call of %s%s%s()",
		fbc->common.scope ? ZSTR_VAL((zend_string *)fbc->common.scope->name) : "",
		fbc->common.scope ? "::" : "",
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_missing_arg_error(zend_execute_data *execute_data)
{
	...
	if (ptr && ptr->func && ZEND_USER_CODE(ptr->func->common.type)) {
		zend_throw_error(zend_ce_argument_count_error, "Too few arguments to function %s%s%s(), %d passed in %s on line %d and %s %d expected",
			EX(func)->common.scope ? ZSTR_VAL((zend_string *)EX(func)->common.scope->name) : "",
...
{
	zend_throw_error(NULL, "Cannot use object of type %s as array", ZSTR_VAL((zend_string *)object->ce->name));
}
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_deprecated_function(const zend_function *fbc)
{
	...
	if (fbc->common.scope) {
		zend_error_unchecked(code, "Method %s::%s() is deprecated%S",
			ZSTR_VAL((zend_string *)fbc->common.scope->name),
			ZSTR_VAL(fbc->common.function_name),
			message_suffix
		);
	} else {
...
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_deprecated_class_constant(const zend_class_constant *c, const zend_string *constant_name)
{
	...
	zend_error_unchecked(code, "%s %s::%s is deprecated%S",
		type,
		ZSTR_VAL((zend_string *)c->ce->name),
		...
		zend_reference *ref, zend_property_info *error_prop OPLINE_DC)
{
	...
	if (ZEND_IS_INCREMENT(opline->opcode)) {
		zend_type_error(
			...
			ZSTR_VAL((zend_string *)error_prop->ce->name),
...
{
	zend_throw_error(NULL, "Call to undefined method %s::%s()", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(method));
}
...
ZEND_COLD void ZEND_FASTCALL zend_param_must_be_ref(const zend_function *func, uint32_t arg_num)
{
	...
	zend_error(E_WARNING, "%s%s%s(): Argument #%d%s%s%s must be passed by reference, value given",
		func->common.scope ? ZSTR_VAL((zend_string *)func->common.scope->name) : "",
		func->common.scope ? "::" : "",
...
fetch_from_array:
	...
	if (UNEXPECTED(Z_TYPE_P(container) == IS_STRING)) {
		...
		if (UNEXPECTED(retval == &EG(uninitialized_zval))) {
			...
			ZVAL_NULL(result);
			zend_error(E_NOTICE, "Indirect modification of overloaded element of %s has no effect", ZSTR_VAL((zend_string *)ce->name));
...
ZEND_API ZEND_COLD void zend_throw_ref_type_error_type(const zend_property_info *prop1, const zend_property_info *prop2, const zval *zv) {
	...
	zend_type_error("Reference with value of type %s held by property %s::$%s of type %s is not compatible with property %s::$%s of type %s",
		...
		ZSTR_VAL((zend_string *)prop1->ce->name),
		zend_get_unmangled_property_name(prop1->name),
		ZSTR_VAL(type1_str),
		ZSTR_VAL((zend_string *)prop2->ce->name),
...
static zend_never_inline ZEND_COLD void ZEND_FASTCALL zend_wrong_clone_call(zend_function *clone, zend_class_entry *scope)
{
	zend_throw_error(NULL, "Call to %s %s::__clone() from %s%s",
		zend_visibility_string(clone->common.fn_flags), ZSTR_VAL((zend_string *)clone->common.scope->name),
...
static zend_never_inline zend_execute_data *zend_init_dynamic_call_object(zend_object *function, uint32_t num_args) /* {{{ */
{
	...
	if (EXPECTED(function->handlers->get_closure) &&
		...
		zend_throw_error(NULL, "Object of type %s is not callable", ZSTR_VAL((zend_string *)function->ce->name));
		return NULL;
	}
...
static zend_never_inline bool ZEND_FASTCALL zend_fe_reset_iterator(zval *array_ptr, int by_ref OPLINE_DC EXECUTE_DATA_DC) /* {{{ */
{
	...
	if (UNEXPECTED(!iter) || UNEXPECTED(EG(exception))) {
		...
		if (!EG(exception)) {
			zend_throw_exception_ex(NULL, 0, "Object of type %s did not create an Iterator", ZSTR_VAL((zend_string *)ce->name));
		}
		ZVAL_UNDEF(EX_VAR(opline->result.var));
		return 1;
	}
