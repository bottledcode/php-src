	...
	if (!fn_proxy || !*fn_proxy) {
		if (EXPECTED(obj_ce)) {
			...
			if (UNEXPECTED(fn == NULL)) {
				/* error at c-level */
				zend_error_noreturn(E_CORE_ERROR, "Couldn't find implementation for method %s::%s", ZSTR_VAL((zend_string *)obj_ce->name), function_name);
			}
		} else {
...
ZEND_API zend_object_iterator *zend_user_it_get_new_iterator(zend_class_entry *ce, zval *object, int by_ref)
{
	...
	if (!ce_it || !ce_it->get_iterator || (ce_it->get_iterator == zend_user_it_get_new_iterator && Z_OBJ(iterator) == Z_OBJ_P(object))) {
		if (!EG(exception)) {
			zend_throw_exception_ex(NULL, 0, "Objects returned by %s::getIterator() must be traversable or implement interface Iterator", ce ? ZSTR_VAL((zend_string *)ce->name) : ZSTR_VAL((zend_string *)Z_OBJCE_P(object)->name));
		}
...
static int zend_implement_traversable(zend_class_entry *interface, zend_class_entry *class_type)
{
	...
	zend_error_noreturn(E_CORE_ERROR, "%s %s must implement interface %s as part of either %s or %s",
		...
		ZSTR_VAL((zend_string *)zend_ce_traversable->name),
		ZSTR_VAL((zend_string *)zend_ce_iterator->name),
		ZSTR_VAL((zend_string *)zend_ce_aggregate->name));
	return FAILURE;
}
/* }}} */
...
ZEND_API int zend_user_serialize(zval *object, unsigned char **buffer, size_t *buf_len, zend_serialize_data *data)
{
	...
	if (result == FAILURE && !EG(exception)) {
		zend_throw_exception_ex(NULL, 0, "%s::serialize() must return a string or NULL", ZSTR_VAL((zend_string *)ce->name));
	}
	return result;
}
/* }}} */
...
static int zend_implement_serializable(zend_class_entry *interface, zend_class_entry *class_type)
{
	...
	if (!(class_type->ce_flags & ZEND_ACC_EXPLICIT_ABSTRACT_CLASS)
		...
		zend_error(E_DEPRECATED, "%s implements the Serializable interface, which is deprecated. Implement __serialize() and __unserialize() instead (or in addition, if support for old PHP versions is necessary)", ZSTR_VAL((zend_string *)class_type->name));
		if (EG(exception)) {
			zend_exception_uncaught_error(
				"During inheritance of %s, while implementing Serializable", ZSTR_VAL((zend_string *)class_type->name));
		}
	}
	return SUCCESS;
}
