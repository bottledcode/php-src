...
#define convert_object_to_type(op, dst, ctype)									\
	...
	if (Z_OBJ_HT_P(op)->cast_object(Z_OBJ_P(op), dst, ctype) == FAILURE) {					\
		zend_error(E_WARNING,																\
			"Object of class %s could not be converted to %s", ZSTR_VAL((zend_string *)Z_OBJCE_P(op)->name),\
...
try_again:
	switch (Z_TYPE_P(op)) {
		...
		case IS_OBJECT: {
			...
			if (!EG(exception)) {
				zend_throw_error(NULL, "Object of class %s could not be converted to string", ZSTR_VAL((zend_string *)Z_OBJCE_P(op)->name));
			}
			zval_ptr_dtor(op);
			ZVAL_EMPTY_STRING(op);
			break;
...
ZEND_API bool ZEND_FASTCALL zend_object_is_true(const zval *op) /* {{{ */
{
	...
	zend_error(E_RECOVERABLE_ERROR, "Object of class %s could not be converted to bool", ZSTR_VAL((zend_string *)zobj->ce->name));
	return false;
}
/* }}} */
