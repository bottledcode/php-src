...
static zend_always_inline void zend_vm_init_call_frame(zend_execute_data *call, uint32_t call_info, zend_function *func, uint32_t num_args, void *object_or_called_scope)
{
	...
	/* Closures created inside a generic frame capture that frame's T-table.
	 * Propagate it onto the call frame so the body's RECV-time checks
	 * resolve outer T-refs. The closure owns the table (persisted=true);
