		 ...
		 && (get == NULL
		  || !(get->common.fn_flags & ZEND_ACC_RETURN_REFERENCE))) {
			zend_throw_error(NULL, "Cannot create reference to property %s::$%s",
				ZSTR_VAL((zend_string *)zobj->ce->name), zend_get_unmangled_property_name(prop_info->name));
			return;
		}
		...
		if (!hooked_iter->by_ref) {
			...
			if (UNEXPECTED(prop_info->flags & ZEND_ACC_READONLY)) {
				zend_throw_error(NULL,
					...
					ZSTR_VAL((zend_string *)prop_info->ce->name), zend_get_unmangled_property_name(prop_info->name));
				return;
			}
