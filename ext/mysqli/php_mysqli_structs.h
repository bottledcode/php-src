...
{ \
	...
	if (my_res->status < __check) { \
		zend_throw_error(NULL, "%s object is not fully initialized", ZSTR_VAL((zend_string *)intern->zo.ce->name)); \
		RETURN_THROWS();\
	}\
}
...
{ \
	...
	if (!(__ptr)->mysql) { \
		zend_throw_error(NULL, "%s object is not fully initialized", ZSTR_VAL((zend_string *)Z_OBJCE_P(__id)->name)); \
		RETURN_THROWS(); \
	} \
}
