...
static char *cli_completion_generator_class(const char *text, size_t textlen, int *state) /* {{{ */
{
	...
	if (retval) {
		rl_completion_append_character = '\0';
		retval = strdup(ZSTR_VAL(ce->namespaced_name.name));
	}
...
*/
	...
	if (text[0] == '$') {
		...
		if (ce && retval) {
			size_t len = ZSTR_LEN(ce->namespaced_name.name) + 2 + strlen(retval) + 1;
			char *tmp = malloc(len);

			snprintf(tmp, len, "%s::%s", ZSTR_VAL(ce->namespaced_name.name), retval);
			free(retval);
			retval = tmp;
		}
	}
