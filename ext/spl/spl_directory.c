...
PHP_METHOD(SplFileInfo, getPathInfo)
{
	...
	if (ce == NULL) {
		...
		zend_argument_type_error(1, "must be a class name derived from %s or null, %s given", ZSTR_VAL(spl_ce_SplFileInfo->namespaced_name.name), ZSTR_VAL(ce->namespaced_name.name));
		RETURN_THROWS();
	}
	...
	if (intern->u.file.func_getCurr->common.scope != spl_ce_SplFileObject) {
		...
		if (Z_TYPE(retval) != IS_STRING) {
			zend_type_error("%s::getCurrentLine(): Return value must be of type string, %s returned",
				ZSTR_VAL(Z_OBJCE_P(this_ptr)->namespaced_name.name), zend_zval_value_name(&retval));
			zval_ptr_dtor(&retval);
			return FAILURE;
		}
