	...
	if (zend_hash_num_elements(&ce->properties_info) > 0) {
		...
		if (unmangled_class == NULL) {
			...
			if (!strcmp(unmangled_class, "*")
			 || !strcasecmp(unmangled_class, ZSTR_VAL(ce->namespaced_name.name))) {
		...
		if (existing_propinfo != NULL) {
			if (!(existing_propinfo->flags & ZEND_ACC_VIRTUAL)) {
				...
				php_error_docref(NULL, E_WARNING,
					...
					ZSTR_VAL(existing_propinfo->ce->namespaced_name.name), Z_STRVAL_P(key));
				zval_ptr_dtor_str(key);
				return -1;
			}
		}
				...
				if (EXPECTED(!ret)) {
					if (UNEXPECTED(obj->ce->ce_flags & ZEND_ACC_NO_DYNAMIC_PROPERTIES)) {
						zend_throw_error(NULL, "Cannot create dynamic property %s::$%s",
							ZSTR_VAL(obj->ce->namespaced_name.name), zend_get_unmangled_property_name(Z_STR_P(&key)));
						zval_ptr_dtor_str(&key);
						goto failure;
...
static inline int object_custom(UNSERIALIZE_PARAMETER, zend_class_entry *ce)
{
	...
	if (ce->unserialize == NULL) {
		zend_error(E_WARNING, "Class %s has no unserializer", ZSTR_VAL(ce->namespaced_name.name));
...
object ":" uiv ":" ["]	{
	...
	*p += 2;
	...
	if (ce->serialize != NULL && !has_unserialize) {
		zend_error(E_WARNING, "Erroneous data format for unserializing '%s'", ZSTR_VAL(ce->namespaced_name.name));
		zend_string_release_ex(class_name, 0);
		return 0;
	}
