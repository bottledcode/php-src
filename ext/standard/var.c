	...
	switch (Z_TYPE_P(struc)) {
		...
		case IS_OBJECT: {
			...
			if (ce->ce_flags & ZEND_ACC_ENUM) {
				...
				php_printf("%senum(%s::%s)\n", COMMON, ZSTR_VAL(ce->namespaced_name.name), Z_STRVAL_P(case_name_zval));
				return;
			}
	...
	switch (Z_TYPE_P(struc)) {
		...
		case IS_OBJECT: {
			...
			if (ce == zend_standard_class_def) {
				...
				smart_str_appendc(buf, '\\');
				smart_str_append(buf, ce->namespaced_name.name);
				if (is_enum) {
					zend_object *zobj = Z_OBJ_P(struc);
...
static HashTable* php_var_serialize_call_sleep(zend_object *obj, zend_function *fn) /* {{{ */
{
	...
	if (Z_TYPE(retval) != IS_ARRAY) {
		zval_ptr_dtor(&retval);
		php_error_docref(NULL, E_WARNING, "%s::__sleep() should return an array only containing the names of instance-variables to serialize", ZSTR_VAL(obj->ce->namespaced_name.name));
		return NULL;
	}
...
static int php_var_serialize_call_magic_serialize(zval *retval, zval *obj) /* {{{ */
{
	...
	if (Z_TYPE_P(retval) != IS_ARRAY) {
		zval_ptr_dtor(retval);
		zend_type_error("%s::__serialize() must return an array", ZSTR_VAL(Z_OBJCE_P(obj)->namespaced_name.name));
		return FAILURE;
	}
		...
		case IS_OBJECT: {
				...
				if (ce->serialize != NULL) {
					...
					if (ce->serialize(struc, &serialized_data, &serialized_length, (zend_serialize_data *)var_hash) == SUCCESS) {
						char b1[32], b2[32];
						char *s1 = zend_print_long_to_buf(b1 + sizeof(b1) - 1, ZSTR_LEN(Z_OBJCE_P(struc)->namespaced_name.name));
						size_t l1 = b1 + sizeof(b1) - 1 - s1;
						char *s2 = zend_print_long_to_buf(b2 + sizeof(b2) - 1, serialized_length);
						size_t l2 = b2 + sizeof(b2) - 1 - s2;
						char *res = smart_str_extend(buf, 2 + l1 + 2 + ZSTR_LEN(Z_OBJCE_P(struc)->namespaced_name.name) + 2 + l2 + 2 + serialized_length + 1);
						res = zend_mempcpy(res, "C:", 2);
						res = zend_mempcpy(res, s1, l1);
						res = zend_mempcpy(res, ":\"", 2);
						res = zend_mempcpy(res, ZSTR_VAL(Z_OBJCE_P(struc)->namespaced_name.name), ZSTR_LEN(Z_OBJCE_P(struc)->namespaced_name.name));
						res = zend_mempcpy(res, "\":", 2);
