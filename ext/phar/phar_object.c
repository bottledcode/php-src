...
static int phar_build(zend_object_iterator *iter, void *puser) /* {{{ */
{
	...
	if (!value) {
		...
		zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned no value", ZSTR_VAL((zend_string *)ce->name));
		return ZEND_HASH_APPLY_STOP;
	}
	...
	switch (Z_TYPE_P(value)) {
		...
		case IS_RESOURCE:
			...
			if (!fp) {
				zend_throw_exception_ex(spl_ce_BadMethodCallException, 0, "Iterator %s returned an invalid stream handle", ZSTR_VAL((zend_string *)ce->name));
				return ZEND_HASH_APPLY_STOP;
			}
			...
			if (iter->funcs->get_current_key) {
				...
				if (Z_TYPE(key) != IS_STRING) {
					zval_ptr_dtor(&key);
					zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned an invalid key (must return a string)", ZSTR_VAL((zend_string *)ce->name));
					return ZEND_HASH_APPLY_STOP;
				}
		...
		case IS_OBJECT:
			if (instanceof_function(Z_OBJCE_P(value), spl_ce_SplFileInfo)) {
				...
				if (!base_len) {
					zend_throw_exception_ex(spl_ce_BadMethodCallException, 0, "Iterator %s returns an SplFileInfo object, so base directory must be specified", ZSTR_VAL((zend_string *)ce->name));
					return ZEND_HASH_APPLY_STOP;
				}
			...
			ZEND_FALLTHROUGH;
		default:
			zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned an invalid value (must return a string, a stream, or an SplFileInfo object)", ZSTR_VAL((zend_string *)ce->name));
			return ZEND_HASH_APPLY_STOP;
	}
...
phar_spl_fileinfo:
	if (base_len) {
		...
		if (fname_len >= base_len && strncmp(fname, base, base_len) == 0 && ((ch = fname[base_len - IS_SLASH(base[base_len - 1])]) == '\0' || IS_SLASH(ch))) {
			...
			zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned a path \"%s\" that is not in the base directory \"%s\"", ZSTR_VAL((zend_string *)ce->name), fname, base);
	...
	if (php_check_open_basedir(fname)) {
		zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned a path \"%s\" that open_basedir prevents opening", ZSTR_VAL((zend_string *)ce->name), fname);
	...
	if (!fp) {
		zend_throw_exception_ex(spl_ce_UnexpectedValueException, 0, "Iterator %s returned a file that could not be opened \"%s\"", ZSTR_VAL((zend_string *)ce->name), fname);
