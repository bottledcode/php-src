...
{
	zend_throw_error(NULL, "Cannot directly construct %s, use document methods instead", ZSTR_VAL((zend_string *)Z_OBJCE_P(ZEND_THIS)->name));
}
...
PHP_METHOD(DOMNode, __sleep)
{
	...
	zend_throw_exception_ex(NULL, 0, "Serialization of '%s' is not allowed, unless serialization methods are implemented in a subclass", ZSTR_VAL((zend_string *)Z_OBJCE_P(ZEND_THIS)->name));
	RETURN_THROWS();
}
...
PHP_METHOD(DOMNode, __wakeup)
{
	...
	zend_throw_exception_ex(NULL, 0, "Unserialization of '%s' is not allowed, unless unserialization methods are implemented in a subclass", ZSTR_VAL((zend_string *)Z_OBJCE_P(ZEND_THIS)->name));
	RETURN_THROWS();
}
