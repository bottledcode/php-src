...
static bool php_xml_check_string_method_arg(
	...
	if (!method_ptr) {
		if (arg_num) {
			zend_argument_value_error(arg_num, "method %s::%s() does not exist", ZSTR_VAL((zend_string *)ce->name), ZSTR_VAL(method_name));
		}
		return false;
	}
...
#define PHP_XML_CHECK_NEW_THIS_METHODS(parser_to_check, new_this_obj, fcc_field, handler_set_method) \
	if ( \
		...
		if (status == false) { \
			...
			ZSTR_VAL((zend_string *)new_this_obj->ce->name), ZSTR_VAL(method_name)); \
